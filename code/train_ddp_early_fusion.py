...
from model.dim3.medformer_early_fusion import RTSuper
import copy
...
def train_net(net, trainset, testset, args, ema_net=None, fold_idx=0):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        stage1 = (epoch < args.train_full_network_epoch) and (not args.resume) and args.pretrain
        
        if stage1 and not did_stage1:
            #train only the first layer for first 10 epochs
            net.module.train_first_layer_only()
            #need to re-wrap DDP after freezing/unfreezing modules
            if args.distributed:
                net = DistributedDataParallel(net.module, device_ids=[args.proc_idx], find_unused_parameters=False)
                # set find_unused_parameters to True if some of the parameters is not used in forward
            did_stage1 = True
        elif (not stage1) and did_stage1:
...
def train_epoch(trainLoader, net, ema_net, optimizer, epoch, writer, args, matcher=None, loss_wrapper=None, mtl_balancer=None):
    ...
    for i, inputs in enumerate(trainLoader):
        ...
        if 'ufo' in args.dataset:
            img = inputs["image"]
            label = inputs["label"]
            unk_voxels = inputs["unk_channels"]
            tumor_volumes_in_crop = inputs["volumes"]
            chosen_segment_mask = inputs["mask"]
            tumor_diameters = inputs["diameters"]
            if "weights" in inputs:
                class_weights = inputs["weights"]
                if isinstance(class_weights, list):
                    if torch.equal(class_weights[0], torch.ones_like(class_weights[0])):
                        class_weights = None
                else:
            ...
            if 'attenuation' in inputs.keys():
                tumor_attenuaton = inputs["attenuation"]
            else:
            ...
            if args.clip_pretrain:
                report_embeddings = inputs['clip_embedding']
            ...
            if 'organ_cropped' in inputs.keys():
                cropped_organs = inputs['organ_cropped']
            else:
                cropped_organs = None
                
            if 'dates' in inputs.keys():
                dates = inputs['dates']
            else:                
                dates = None
        ...
        if args.aug_device != 'gpu':
            if isinstance(img, list):
                img = [i.float().cuda(non_blocking=True) for i in img]
            else:
                img = img.float().cuda(non_blocking=True)
            if isinstance(label, list):
                label = [l.long().cuda(non_blocking=True) for l in label]
            else:
                label = label.long().cuda(non_blocking=True)
            if unk_voxels is not None:
                if isinstance(unk_voxels, list):
                    unk_voxels = [u.float().cuda(non_blocking=True) for u in unk_voxels]
                else:
                    unk_voxels = unk_voxels.float().cuda(non_blocking=True)
            if tumor_volumes_in_crop is not None:
                if isinstance(tumor_volumes_in_crop, list):
                    tumor_volumes_in_crop = [t.float().cuda(non_blocking=True) for t in tumor_volumes_in_crop]
                    if not args.use_all_data:
                        assert (tumor_volumes_in_crop[0].sum(dim=-1)>=0).all(), 'There are samples without tumor volume in the batch, use --use_all_data to accept tumors of unknown size (marked as -999999 volume)'
                else:
                    tumor_volumes_in_crop = tumor_volumes_in_crop.float().cuda(non_blocking=True)
                    if not args.use_all_data:
            ...
            if chosen_segment_mask is not None:
                if isinstance(chosen_segment_mask, list):
                    chosen_segment_mask = [c.float().cuda(non_blocking=True) for c in chosen_segment_mask]
                else:
                    chosen_segment_mask = chosen_segment_mask.float().cuda(non_blocking=True)
            if tumor_diameters is not None:
                if isinstance(tumor_diameters, list):
                    tumor_diameters = [t.float().cuda(non_blocking=True) for t in tumor_diameters]
                else:
                    tumor_diameters = tumor_diameters.float().cuda(non_blocking=True)
       
        step = i + epoch * len(trainLoader) # global steps
        
        optimizer.zero_grad(set_to_none=True)
        if isinstance(img, list):
            assert not torch.isnan(img[0]).any(), 'Input is nan'
            assert torch.max(img[0])<=100, f'Input is bigger than 100: {torch.max(img[0])}'
            assert torch.min(img[0])>=-100, f'Input is smaller than -100: {torch.min(img[0])}'
        else:
        ...
        lession_class_names = trainLoader.dataset.lession_class_names if hasattr(trainLoader.dataset, 'lession_class_names') else None
        ...
        if args.amp:
            with torch.autocast(device_type='cuda', dtype=torch.bfloat16): #do not use float16, unstable
                ...
                result = net(img, tumor_info=tumor_info, stage_1_out = stage_1, labels = label, cut_attenuation_grad = (epoch < (args.warmup*4)),name=names,
                             ...
                             age=age,sex=sex,dates = dates, cropped_organs = cropped_organs) 
                if isinstance(result, dict) and 'time_point_0' in list(result.keys()):
                    losses = []
                    losses_all = []
                    for t,key in enumerate(result.keys(),0):
                        assert 'time_point' in key, f'was expecting only time point keys, got {result.keys()}'
                        loss, loss_all = lf.calculate_loss_teacher_student(
                                            result=result[key], 
                                            label=label[t].long(), 
                                            unk_voxels=unk_voxels[t], 
                                            args=args,
                                            matcher=matcher,
                                            chosen_segment_mask=chosen_segment_mask[t],
                                            tumor_volumes_in_crop=tumor_volumes_in_crop[t], 
                                            tumor_diameters=tumor_diameters[t],
                                            classes=classes,
                                            img=img[t],
                                            class_weights=class_weights[t].float() if class_weights is not None else None,
                                            report_embeddings=report_embeddings[t].float() if report_embeddings is not None else None, 
                                            dist=dist,
                                            tumor_attenuaton=tumor_attenuaton[t].float() if tumor_attenuaton is not None else None,
                                            lession_class_names=lession_class_names,
                                            tumor_volumes_in_crop_per_voxel=tumor_volumes_in_crop_per_voxel[t] if tumor_volumes_in_crop_per_voxel is not None else None,
                                            tumor_diameters_per_voxel=tumor_diameters_per_voxel[t] if tumor_diameters_per_voxel is not None else None,
                                            names=names[t] if names is not None else None,
                                            slices_cropped_dict=slices_cropped_dict[t] if slices_cropped_dict is not None else None, 
                                            sizes_slices=sizes_slices[t] if sizes_slices is not None else None,
                                            sizes_malignancy=sizes_malignancy[t] if sizes_malignancy is not None else None, 
                                            malignancy_per_voxel=malignancy_per_voxel[t] if malignancy_per_voxel is not None else None,
                                            contrast=contrast[t] if contrast is not None else None,)
                        losses.append(loss)
                        losses_all.append(loss_all)
                    losses = torch.stack(losses,dim=0).mean(0)
                    losses_all = {k: torch.stack([loss_all[k] for loss_all in losses_all],dim=0).mean(0) for k in losses_all[0].keys()}
                else:
                    loss, loss_all = lf.calculate_loss_teacher_student(
                        result=result, 
                        label=label, 
                        ...
                        dist=dist,
                        tumor_attenuaton=tumor_attenuaton.float(),
                        lession_class_names=lession_class_names,
        ...
        loss.backward()
        #debug: do we have parameters without gradients?
        ...
        for k, v in loss_all.items():
            loss_meters[k].update(v.detach().item(), bs)
...
def get_parser():
    ...
    parser.add_argument('--load_per_patient', action='store_true', help='loads data per patient.')
    parser.add_argument('--maximum_loaded_per_patient', type=int, default=None, help='if loading per patient, maximum number of samples to load per patient')
    ...
    parser.add_argument('--time_fusion', type=str, default='early', help='type of time fusion: early or late')
...
def init_network(args,classes=None,old_classes=None):
    ...
    if args.time_points > 1 and args.time_fusion == 'early':
        teacher_student.make_longitudinal(
            time_points=args.time_points, time_fusion=args.time_fusion,
            num_inpt_ch=1,
            use_transformer_decoder=not(args.remove_transformer_decoder),
            use_dynamic_conv=not(args.remove_dynamic_conv),
            age_and_sex_provided=args.age_and_sex_provided,
