    ...
    with torch.no_grad():
        ...
        if D > z_len:
            ...
            for i in range(num_z_chunks):
                ...
                pred = inference(model, image_chunk, args, pancreas=tgt_organ, use_transformer_decoder=args.use_transformer_decoder,
                                 age = age, sex = sex)
            ...
            pred = inference(model, tensor_img, args, pancreas=tgt_organ,use_transformer_decoder=args.use_transformer_decoder,
                                 age = age, sex = sex)
            if isinstance(pred, list) or isinstance(pred, tuple):
                cls_out = pred[1]
                pred = pred[0]
...
if __name__ == '__main__':
    ...
    for img_name in tqdm.tqdm(ids):
        ...
        pred_label, pred_raw, cls_out = prediction(model_list, tensor_img, args, tgt_organ=pancreas, age=age, sex=sex)    
        #try:
