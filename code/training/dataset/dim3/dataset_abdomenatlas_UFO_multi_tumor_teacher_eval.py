...
class AbdomenAtlasDataset(Dataset):
    ...
    def getitem_real(self, idx, name=None, BDMAP_ID=None, debug_dict=False,try_2=False,
        ...
        if self.mode == 'train':
            ...
            # TEACHER-EVAL: surface the crop coordinates [z0,z1,y0,y1,x0,x1]
            # for paste-back in stage-2 inference. None if no preferential crop
            # was performed (e.g., tumor-based crop or random crop).
            if isinstance(slices_cropped_dict, dict) and 'crop_coords' in slices_cropped_dict:
                retur['crop_coords'] = slices_cropped_dict['crop_coords']
            else:
                retur['crop_coords'] = None
    ...
    def random_crop_on_segment(self, tensor_img, tensor_lab, tumor_segment_mask, d, h, w,
        ...
        # TEACHER-EVAL DATASET COPY: deterministic, augmentation-free crop. We
        # take the centered (rand=False) crop so the cropped CT can be pasted
        # back into the full-resolution prediction at known coordinates. The
        # original dataset's random rotate/translate branch (was gated by
        # `np.random.random() < 0.4`) is intentionally removed for inference.
        # `return_coordinate=True` gives us [z0,z1,y0,y1,x0,x1] in the original
        # CT frame, surfaced via slices_cropped_dict['crop_coords'] below.
        ...
        if isinstance(out, tuple):
            tensor_img, tensor_lab, tumor_segment_mask, crop_coords = out
            if tumor_segment_mask.sum()==0:
            ...
            if slices_mask is not None:
                ...
                slices_cropped_dict={'slices_mask': slices_mask,
                                     ...
                                     'crop_coords': tuple(crop_coords), #TEACHER-EVAL: [z0,z1,y0,y1,x0,x1] in original CT frame, for paste-back
                                     }

            else:
                # TEACHER-EVAL: keep a minimal slices_cropped_dict so we can
                # surface crop_coords even when slices weren't used. Defaults
                # mirror the no-slices path that getitem_real uses (~line 4002
                # of the original).
                slices_cropped_dict = {
                    'slices_mask': torch.zeros_like(tensor_img).float(),
                    'tumor_df': '[]',
                    'binary_slices_mask': torch.ones_like(tensor_img).float(),
                    'max_slice': 0,
                    'slices_used': False,
                    'z_spacing': 0.0,
    ...
    def get_input_info_report(self, retur, idx):
        ...
        if not annotated_per_voxel:
            ...
            volumes = retur['volumes']
            tumor_count = (volumes!=0).sum().item()
            if tumor_count ==0:
                #no tumor
