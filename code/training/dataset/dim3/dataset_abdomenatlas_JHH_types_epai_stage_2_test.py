...
def isolate_components_and_sample_coords(x):
    """
    Given a binary N-D tensor `x` (only 0s and 1s), find all connected components 
    of the 1s, attempt to erode each component’s border, sample one random voxel 
    from each (using the eroded mask if non-empty, else the original), and then 
    return the sampled coordinates ordered by component size (largest → smallest).

    Args:
        x (torch.Tensor): binary tensor (0/1) of shape (...).

    Returns:
        List[Tuple[int, ...]]: sampled coordinate per component, ordered by the 
        original component size descending.
    """
    # 1) sanity-check that x is binary
    vals = torch.unique(x)
    if not set(vals.tolist()).issubset({0, 1}):
        raise ValueError(f"Input must be binary (0/1), got values {vals.tolist()}")

    # 2) label components
    x_np = x.cpu().numpy().astype(np.int32)
    labeled, n_comp = cc_label(x_np)

    # 3) collect (size, coord) for each component
    size_coord_list = []
    for comp_id in range(1, n_comp + 1):
        # original mask and its size
        orig_mask = (labeled == comp_id).astype(np.uint8)
        comp_size = int(orig_mask.sum())
        if comp_size == 0:
            continue  # shouldn't happen, but guard anyway

        # try erosion
        eroded = augmentation.denoise_mask(orig_mask, iterations=4, connected_component=False)
        mask = eroded if eroded.sum() > 0 else orig_mask

        # sample one random point
        pts = np.argwhere(mask)
        idx = np.random.randint(len(pts))
        coord = tuple(int(v) for v in pts[idx])

        size_coord_list.append((comp_size, coord))

    # 4) sort by size descending and return coords
    size_coord_list.sort(key=lambda sc: sc[0], reverse=True)
    ordered_coords = [coord for _, coord in size_coord_list]
    return ordered_coords

class AbdomenAtlasDataset(Dataset):
    def __init__(self, args, mode='test', seed=0, all_train=False,
                crop_on_tumor=True,
        ...
        if isinstance(id_list, str):
            self.img_name_list_jhh=pd.read_csv(id_list)['BDMAP_ID'].tolist()
        else:
            self.img_name_list_jhh=id_list
        ...
        if id_list is None and mode == 'test':
            raise ValueError('id_list is required for test mode')

        print(f'Id list length: {len(img_name_list)}')
        ...
        if mode == 'train':
            self.tumor_crop_probability = 0.9
        else:
            self.tumor_crop_probability = 1.0
            if self.load_augmented:
                raise ValueError('load_augmented=True but mode=test. Cannot load augmented data in test mode.')
    ...
    def __getitem__(self, idx):
        ...
        stage_1 = np.load(self.stage_1[idx], mmap_mode='r', allow_pickle=False)['arr_0']
        try:
        ...
        if not tumor_presence:
            ...
            if foreground_mask.sum() == 0:
                ...
                if random.random() < (1-self.tumor_crop_probability) or foreground_mask.sum()==0:#random crop on background
            ...
            if self.mode == 'train':
                ...
                #crop on each of the tumors in stage 1
                crop_coordinates = isolate_components_and_sample_coords(stage_1_binary)
                img_list = []
                lab_list = []
                
                #print('Shape of image and label:', tensor_img.shape, tensor_lab.shape, flush=True, file=sys.stderr)
                


                C_img = tensor_img.shape[1]
                C_lab = tensor_lab.shape[1]
                device_img = tensor_img.device
                device_lab = tensor_lab.device
                dtype_img = tensor_img.dtype
                dtype_lab = tensor_lab.dtype

                for i in range(3):
                    if i < len(crop_coordinates):
                        coord = crop_coordinates[i]
                        #print('Coordinate:', coord, flush=True, file=sys.stderr)
                        #print('image shape:', tensor_img.shape, flush=True, file=sys.stderr)
                        img_crop, lab_crop = augmentation.crop_around_coordinate_3d(
                            tensor_img, tensor_lab,
                            [d, h, w],
                            coordinate=(coord[-3],coord[-2],coord[-1]),
                            mode='center'
                        )
                        #print('Crop shape:', img_crop.shape, lab_crop.shape, flush=True, file=sys.stderr)
                    #else: only pad if you need to use a data loader, no need to pad if you use getitem
                        # make a zero‐crop of the same shape as an actual crop
                    #    img_crop = torch.zeros((1,C_img, d, h, w), device=device_img, dtype=dtype_img)
                    #    lab_crop = torch.zeros((1,C_lab, d, h, w), device=device_lab, dtype=dtype_lab)
                        #print('Zero crop shape:', img_crop.shape, lab_crop.shape, flush=True, file=sys.stderr)
                        img_list.append(img_crop.squeeze(0))
                        lab_list.append(lab_crop.squeeze(0))
                    
                tensor_img = torch.stack(img_list, dim=0)
                tensor_lab = torch.stack(lab_list, dim=0)
                
        if len(tensor_img.shape) == 4 and len(tensor_lab.shape) == 4:
            tensor_img = tensor_img.unsqueeze(0)
            tensor_lab = tensor_lab.unsqueeze(0)
        elif len(tensor_img.shape) == 5 and len(tensor_lab.shape) == 5:
            pass
        else:
            raise ValueError('Image and label should have 4 or 5 dimensions, got %s and %s'%(tensor_img.shape, tensor_lab.shape))


        tensor_img, tensor_lab = tensor_img.contiguous(), tensor_lab.contiguous()

        if self.mode == 'train' and (not self.save_augmented):
        ...
        if self.mode == 'train':
            ...
            return tensor_img.float(), tensor_lab.float(), np.array(self.spacing_list[idx]), self.img_list[idx]#name
    ...
    def save_sanity_check(self, img, lab, idx):
        ...
        if self.saved_count < 10:
            save_dir = './SanityCheckEPAIStage2Test'
            ...
            # Save the image
            img_nifti = sitk.GetImageFromArray(img[0].squeeze().cpu().numpy())
            #print shape
            ...
            for b in range(lab.shape[0]):
                ...
                for i, cls in enumerate(self.label_order+['stage_1']):
                    label_array = (l[i].squeeze().cpu().numpy())
                    ...
                    sitk.WriteImage(label_nifti, os.path.join(img_folder, f'{str(b)}_{cls}.nii.gz'))
