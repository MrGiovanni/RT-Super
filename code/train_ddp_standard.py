...
from training.losses import DiceLossMultiClass
...
def train_epoch(trainLoader, net, ema_net, optimizer, epoch, writer, criterion, criterion_dl, scaler, args, matcher=None):
            ...
            if isinstance(result, tuple) or isinstance(result, list):
                ...
                for j in range(len(result)):
                    if j==0 and args.multi_ch_tumor:
                        ...
                        out_ids, label_ids = matcher(result[j], label)
                    if args.multi_ch_tumor:
                        ...
                        r=result[j]
                        l=label
                    loss += args.aux_weight[j] * (criterion(r, l.float()) + criterion_dl(r, l))
            else:
                if args.multi_ch_tumor:
                    out_ids, label_ids = matcher(result, label)
                    result=result[out_ids]
                    label=label[label_ids]
                loss = criterion(result, label.float()) + criterion_dl(result, label)
            ...
            if isinstance(result, tuple) or isinstance(result, list):
                ...
                for j in range(len(result)):
                    ...
                    if args.multi_ch_tumor:
                        ...
                        r=result[j]
                        l=label
                    CE=criterion(r, l.float())
                    CE=args.aux_weight[j]*CE
                    loss += args.aux_weight[j] * (CE + criterion_dl(r, l))
            else:
                if args.multi_ch_tumor:
                    out_ids, label_ids = matcher(result, label)
                    result=result[out_ids]
...
def get_parser():
    ...
    parser.add_argument('--use_k_fold', action='store_true', help='uses k fold cross validation')
    ...
    parser.add_argument('--multi_ch_tumor_data_root', type=str, default='/fastwork/psalvador/JHU/data/atlas_300_medformer_multi_ch_tumor_npy/', help='data root for multi channel tumor dataset')
    ...
    parser.add_argument('--workers_overwrite', type=int, default=None, help='overwrites number of workers in config file') 
    ...
    parser.add_argument('--dist_url', type=str, default='tcp://127.0.0.1:12345', help='url used to set up distributed training')
    ...
    if args.workers_overwrite is not None:
        args.num_workers = args.workers_overwrite
        print(f'Overwriting number of workers to {args.num_workers}')
